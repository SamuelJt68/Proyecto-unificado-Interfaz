	...
	private  InterfazEmpresa interfaz;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (grito.equals("Iniciar sesion")) {
				...
				if (interfaz.ValidarAdmin(usuario, contrasena)&&(interfaz.getEmpresa().getUsuarioActual().getClass().getSimpleName().equals("Administrador"))) {
					interfaz.menuAdmi();
				}
				else if (interfaz.ValidarAdminLocal(usuario, contrasena)&&(interfaz.getEmpresa().getUsuarioActual().getClass().getSimpleName().equals("AdminLocal"))) {
					interfaz.menuAdmiLocal();
				}
				else if (interfaz.ValidarEmpleado(usuario,contrasena)&&(interfaz.getEmpresa().getUsuarioActual().getClass().getSimpleName().equals("Empleado"))) {
					interfaz.menuEmpleado();
				}				
				else {
					JOptionPane.showMessageDialog(this,"El usuario o contraseña son incorrectos"+interfaz.ValidarAdmin(usuario, contrasena)+interfaz.getEmpresa().getUsuarioActual().getClass().getSimpleName().equals("AdminLocal"), "Usuario no encontrado", JOptionPane.INFORMATION_MESSAGE );
				}
			}
